...
Multiple expressions are separated by semicolons. All must evaluate to `true` for the test to pass.
...
```scaf
assert {
  // Won't error if profile is nil
  u.profile == nil || u.profile.bio != ""
}
```
...
assert {
  ...
  // Verified users must have email
